...
cl::opt<std::string> RunInDir("run-in",
		cl::desc("Change to the given directory prior to executing"));

cl::opt<std::string> Environ("environ",
...
cl::opt<bool> WarnAllExternals("warn-all-externals",
...
cl::opt<bool> WriteSMT2s("write-smt2s",
		cl::desc("Write .smt2 (SMT-LIBv2) files for each test case"));

cl::opt<bool> WriteCov("write-cov",
...
cl::opt<bool> WriteSymPaths("write-sym-paths",
...
cl::opt<LibcType> Libc("libc",
		...
		cl::values(clEnumValN(NoLibc, "none", "Don't link in a libc"),
				...
				clEnumValN(UcLibc, "uclibc",
						"Link in uclibc (adapted for klee)"), clEnumValEnd),
		cl::init(NoLibc));
...
cl::opt<bool> CheckDivZero("check-div-zero",
		cl::desc("Inject checks for division-by-zero"), cl::init(true));

cl::opt<bool> CheckOvershift("check-overshift",
		cl::desc("Inject checks for overshift"), cl::init(true));
...
cl::list<std::string> ReadArgsFilesFake("read-args",
		cl::desc("File to read arguments from (one arg per line)"));

cl::opt<bool> ReplayKeepSymbolic("replay-keep-symbolic",
		cl::desc("Replay the test cases only by asserting "
				"the bytes, not necessarily making them concrete."));

cl::list<std::string> ReplayOutFile("replay-out",
		cl::desc("Specify an out file to replay"), cl::value_desc("out file"));

cl::list<std::string> ReplayOutDir("replay-out-dir",
...
cl::opt<std::string> ReplayPathFile("replay-path",
		cl::desc("Specify a path file to replay"), cl::value_desc("path file"));
...
cl::opt<bool> Watchdog("watchdog",
		cl::desc("Use a watchdog process to enforce --max-time."), cl::init(0));
}
...
KleeHandler::KleeHandler(int argc, char **argv) :
		m_interpreter(0), m_pathWriter(0), m_symPathWriter(0), m_infoFile(0), m_outputDirectory(), m_testIndex(
				0), m_pathsExplored(0), m_argc(argc), m_argv(argv) {
	...
	if ((ec = sys::fs::make_absolute(directory)) != errc::success)
		klee_error("unable to determine absolute path: %s",
				ec.message().c_str());
		...
		for (; i <= INT_MAX; ++i) {
			...
			if (mkdir(d.c_str(), 0775) == 0) {
				...
				if (((unlink(klee_last.c_str()) < 0) && (errno != ENOENT))
						|| symlink(m_outputDirectory.c_str(), klee_last.c_str())
								< 0) {
			...
			if (errno != EEXIST)
				klee_error("cannot create \"%s\": %s",
						m_outputDirectory.c_str(), strerror(errno));
		}
...
void KleeHandler::setInterpreter(Interpreter *i) {
	...
	if (WriteSymPaths) {
		m_symPathWriter = new TreeStreamWriter(
				getOutputFilename("symPaths.ts"));
...
std::ostream *KleeHandler::openOutputFile(const std::string &filename) {
	...
	if (!f) {
		klee_error("error opening file \"%s\" (out of memory)",
				filename.c_str());
	} else if (!f->good()) {
		klee_error(
				"error opening file \"%s\".  KLEE may have run out of file descriptors: try to increase the maximum number of open file descriptors by using ulimit.",
				filename.c_str());
		delete f;
		f = NULL;
	}
...
void KleeHandler::getOutFiles(std::string path,
		std::vector<std::string> &results) {
	error_code ec;
	for (llvm::sys::fs::directory_iterator i(path, ec), e; i != e && !ec;
			i.increment(ec)) {
		std::string f = (*i).path();
...
std::string KleeHandler::getRunTimeLibraryPath(const char* argv0,
	...
	if (strcmp(toolRoot.c_str(), KLEE_INSTALL_BIN_DIR) == 0) {
		DEBUG_WITH_TYPE("klee_runtime",
				llvm::dbgs() << "Using installed KLEE library runtime: ");
		libDir = KLEE_INSTALL_LIB_DIR;
	} else {
		DEBUG_WITH_TYPE("klee_runtime",
				llvm::dbgs() << "Using build directory KLEE library runtime :");
		libDir = KLEE_DIR;
		llvm::sys::path::append(libDir, RUNTIME_CONFIGURATION);
		llvm::sys::path::append(libDir, "lib");
	}

	DEBUG_WITH_TYPE("klee_runtime", llvm::dbgs() << libDir.c_str() << "\n");
	return libDir.str();
}
...
static int initEnv(Module *mainModule) {
	...
	if (mainFn->arg_size() < 2) {
		klee_error(
				"Cannot handle " "--posix-runtime" " when main() has less than two arguments.\n");
	}
	...
	params.push_back(Type::getInt32Ty(getGlobalContext()));
	Function* initEnvFn = cast < Function
			> (mainModule->getOrInsertFunction("klee_init_env",
					Type::getVoidTy(getGlobalContext()), argcPtr->getType(),
					argvPtr->getType(),
					NULL));
	assert(initEnvFn);
...
static const char *modelledExternals[] = {
		...
		"klee_get_errno", "klee_get_valuef", "klee_get_valued",
		"klee_get_valuel", "klee_get_valuell", "klee_get_value_i32",
		"klee_get_value_i64", "klee_get_obj_size", "klee_is_symbolic",
		"klee_make_symbolic", "klee_mark_global", "klee_merge",
		"klee_prefer_cex", "klee_print_expr", "klee_print_range",
...
static const char *dontCareKlee[] = { "__ctype_b_loc", "__ctype_get_mb_cur_max",
...
void externalsAndGlobalsCheck(const Module *m) {
	...
	for (std::map<std::string, bool>::iterator it = externals.begin(), ie =
		...
		if (!modelled.count(ext)
				&& (WarnAllExternals || !dontCare.count(ext))) {
			if (unsafe.count(ext)) {
...
*/
//	mainModule->dump();
	std::cerr << "string = " << uclibcBCA.c_str() << std::endl;
...
int main(int argc, char **argv, char **envp) {
	...
	//create debug info folder
	int ret = system("rm -fr output_info ; mkdir output_info");
	assert(WIFEXITED(ret) != 0 && "exit abnormally!");
			...
			while (1) {
				...
				if (res < 0) {
					...
					if (time > nextStep) {
						...
						nextStep = util::getWallTime()
								+ std::max(15., MaxTime * .1);
					}
				}
			}
	...
	mainModule = getLazyBitcodeModule(BufferPtr.get(), getGlobalContext(),
			&ErrorMsg);
	...
	Interpreter *interpreter = theInterpreter = Interpreter::create(IOpts,
			handler);
...
//	mainModule->dump();
	...
	if (!ReplayOutDir.empty() || !ReplayOutFile.empty()) {
		...
		for (std::vector<std::string>::iterator it = ReplayOutDir.begin(), ie =
				ReplayOutDir.end(); it != ie; ++it)
			KleeHandler::getOutFiles(*it, outFiles);
		std::vector<KTest*> kTests;
		for (std::vector<std::string>::iterator it = outFiles.begin(), ie =
				outFiles.end(); it != ie; ++it) {
		...
		if (RunInDir != "") {
			...
			if (res < 0) {
				klee_error("Unable to change directory to: %s",
						RunInDir.c_str());
			}
		}

		unsigned i = 0;
		for (std::vector<KTest*>::iterator it = kTests.begin(), ie =
				kTests.end(); it != ie; ++it) {
			KTest *out = *it;
			interpreter->setReplayOut(out);
			std::cerr << "KLEE: replaying: " << *it << " ("
					<< kTest_numBytes(out) << " bytes)" << " (" << ++i << "/"
					<< outFiles.size() << ")\n";
			// XXX should put envp in .ktest ?
			interpreter->runFunctionAsMain(mainFn, out->numArgs, out->args,
					pEnvp);
			if (interrupted)
				break;
		}
		...
		for (std::vector<std::string>::iterator it = SeedOutFile.begin(), ie =
				SeedOutFile.end(); it != ie; ++it) {
			KTest *out = kTest_fromFile(it->c_str());
			if (!out) {
				std::cerr << "KLEE: unable to open: " << *it << "\n";
				exit(1);
		...
		for (std::vector<std::string>::iterator it = SeedOutDir.begin(), ie =
				SeedOutDir.end(); it != ie; ++it) {
			std::vector<std::string> outFiles;
			KleeHandler::getOutFiles(*it, outFiles);
			for (std::vector<std::string>::iterator it2 = outFiles.begin(), ie =
					outFiles.end(); it2 != ie; ++it2) {
