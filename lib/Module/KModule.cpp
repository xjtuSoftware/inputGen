...
void KModule::prepare(const Interpreter::ModuleOptions &opts,
  ...
  std::set<std::string> kleeFunctionNames;
  std::set<std::string> intrinsicFunctionNames;

  module = linkWithLibrary(module, LibPath.str(), &kleeFunctionNames, &intrinsicFunctionNames);

  for (std::set<std::string>::iterator ni = kleeFunctionNames.begin(), ne = kleeFunctionNames.end(); ni != ne; ni++) {
	  StringRef functionName(*ni);
	  Function* function = module->getFunction(functionName);
	  if (function) {
		  kleeFunctions.insert(function);
	  }
  }

  for (std::set<std::string>::iterator ni = intrinsicFunctionNames.begin(), ne = intrinsicFunctionNames.end(); ni != ne; ni++) {
	  StringRef functionName(*ni);
	  Function* function = module->getFunction(functionName);
  	  if (function) {
  		  intrinsicFunctions.insert(function);
  	  }
  }
