...
//  std::cerr << "expr->getWidth = " << expr->getWidth()
//		  << ", offSet = " << off << ", Width = " << w << std::endl;
...
static ref<Expr> AddExpr_create(Expr *l, Expr *r) {
  ...
  if (type == Expr::Bool) {
    ...
    if (lk==Expr::Add && isa<ConstantExpr>(l->getKid(0))) { // (k+a)+b = k+(a+b)
    	if (r->isFloat || l->getKid(0)->isFloat || l->getKid(1)->isFloat) {
    	    	    	  l->getKid(0)->isFloat = true;
    	    	    	  l->getKid(1)->isFloat = true;
    	    	    	  r->isFloat = true;
    	}
    	...
    	if (l->isFloat || r->getKid(0)->isFloat || r->getKid(1)->isFloat) {
    	    	  r->getKid(0)->isFloat = true;
    	    	  r->getKid(1)->isFloat = true;
    	    	  l->isFloat = true;
    	      }
...
static ref<Expr> SubExpr_createPartial(Expr *l, const ref<ConstantExpr> &cr) {
  // l - c => l + (-c)
	ref<ConstantExpr> ce = ConstantExpr::alloc(0, cr->getWidth())->Sub(cr);
  if (cr.get()->isFloat) {
	 ce.get()->isFloat = true;
  }
	return AddExpr_createPartial(l, ce);
}
